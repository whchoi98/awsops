...
- **Steampipe Aggregator**: search_path 전환으로 어카운트 스코핑 (SQL 쿼리 변경 불필요)
- **Account Selector**: Sidebar 드롭다운으로 "All Accounts" / 개별 어카운트 전환
- **Accounts 페이지** (`/accounts`): UI에서 어카운트 추가/삭제/연결 테스트, CFN 명령 안내
- **자동 리프레시**: 어카운트 전환 시 모든 페이지 데이터 자동 갱신 (useCallback 의존성)
- **Account 컬럼**: DataTable이 멀티 어카운트 모드에서 account_id 컬럼 자동 표시
- **Detail 패널**: 모든 리소스 상세 패널에 Account 정보 조건부 표시
- **Features 기반 메뉴**: 어카운트별 costEnabled/eksEnabled로 Sidebar 메뉴 동적 표시
- **CFN 템플릿**: `infra-cdk/cfn-target-account-role.yaml`로 Target 어카운트 IAM Role 자동 생성
- **Lambda cross-account**: `cross_account.py` 헬퍼 + 125개 도구에 target_account_id 파라미터
- **CloudWatch API**: 4개 서비스(RDS/MSK/ElastiCache/OpenSearch)에 --profile 지원
- **Pool 리셋**: Steampipe 재시작 후 pg Pool 자동 리셋 + 헬스체크 대기
- **하위 호환**: accounts 미설정 시 기존 동작 100% 유지

#### English

#### Multi-Account Support
- **Cross-Account IAM Role**: Deploy on host account, access target accounts via AssumeRole
- **Steampipe Aggregator**: search_path switching for account scoping (no SQL query changes needed)
- **Account Selector**: Sidebar dropdown for "All Accounts" / individual account switching
- **Accounts Page** (`/accounts`): Add/remove/test accounts from UI with CFN command guidance
- **Auto-refresh**: All pages auto-refresh on account switch (useCallback dependency)
- **Account Column**: DataTable auto-displays account_id column in multi-account mode
- **Detail Panels**: All resource detail panels show Account info conditionally
- **Feature-based Menu**: Sidebar menus dynamically show/hide based on per-account features
- **CFN Template**: `infra-cdk/cfn-target-account-role.yaml` for automated target account IAM Role creation
- **Lambda cross-account**: `cross_account.py` helper + target_account_id parameter on 125 tools
- **CloudWatch APIs**: --profile support for RDS/MSK/ElastiCache/OpenSearch
- **Pool Reset**: Auto pg Pool reset after Steampipe restart with health check
- **Backward Compatible**: Zero behavioral change when accounts not configured
