...
- `queries/*.ts` — 24개 SQL 쿼리 파일 (ebs, msk, opensearch, container-cost, eks-container-cost 포함, list/detail에 account_id 포함)
- `resource-inventory.ts` — 리소스 인벤토리 스냅샷 (data/inventory/{accountId}/, 추가 쿼리 0건)
- `cost-snapshot.ts` — Cost 데이터 스냅샷 폴백 (data/cost/{accountId}/)
- `app-config.ts` — 앱 설정 + **AccountConfig, getAccounts(), isMultiAccount(), getAccountFeatures()** + FargatePricing, opencostEndpoint
...
- `app-config.ts` — App config + **AccountConfig, getAccounts(), isMultiAccount(), getAccountFeatures()** + FargatePricing, opencostEndpoint
