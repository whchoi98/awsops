...
| **agent.py** | 계정별 Gateway URL 업데이트 후 Docker 재빌드 필요 |
| **Code Interpreter** | 이름에 하이픈 불가, 언더스코어만 사용 |
| **Memory Store** | 이름에 하이픈 불가 (`awsops_memory`), 최대 365일 보관 |
| **Runtime 업데이트** | `--role-arn` + `--network-configuration` 필수 |
...
| **대화 이력 보관** | 최대 365일 |
| **AgentCore 응답** | 최종 텍스트만 반환 (도구 추론, 타이핑 효과로 스트리밍) |
